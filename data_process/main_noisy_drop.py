...
_category = 3 # Benign、DDoS、Recon
# 修改 1：新建一个独立文件夹存放噪声 TSV，防止覆盖原文件
dataset_dir = "/mnt/nfs/kaleid/ET-BERT/datasets_noisy/" 

# 修改 2：指向你的 Drop 增强噪声 PCAP 文件夹
pcap_path = "/mnt/nfs/kaleid/ET-BERT/dataset_noisy_drop/" 

# 修改 3：必须修改缓存目录！否则代码会直接读取原来干净数据的 .npy 缓存，根本不去解析新的 PCAP
dataset_save_path = "/mnt/nfs/kaleid/ET-BERT/ex_results_noisy_drop/"
samples = [5000]         # 每类最多提取 5000 个流
features = ["length"]    # 特征包长
dataset_level = "flow"   # 按流(Session)提取
...
def dataset_extract(model):
    ...
    try:
        if os.path.exists(dataset_save_path + "dataset/") and os.listdir(dataset_save_path + "dataset/"):
            print("Reading dataset from %s ..." % (dataset_save_path + "dataset/"))
            
            x_length_train, x_length_test, x_length_valid,\
            y_train, y_test, y_valid = \
                np.load(dataset_save_path + "dataset/x_length_train.npy",allow_pickle=True), np.load(dataset_save_path + "dataset/x_length_test.npy",allow_pickle=True), np.load(dataset_save_path + "dataset/x_length_valid.npy",allow_pickle=True),\
        ...
        if features[i] == "length":
            for index_label in range(len(X[0])):
                for index_sample in range(len(X[0][index_label])):
                    X_length.append(X[0][index_label][index_sample])
    ...
    for test_index, valid_index in split_2.split(x_length_test, y_test):
        ...
        x_length_test, y_test = x_length_test[test_index], y_test[test_index]

    if not os.path.exists(dataset_save_path+"dataset/"):
        os.makedirs(dataset_save_path+"dataset/")

    output_x_length_train = os.path.join(dataset_save_path + "dataset/", 'x_length_train.npy')
    output_x_length_test = os.path.join(dataset_save_path + "dataset/", 'x_length_test.npy')
    output_x_length_valid = os.path.join(dataset_save_path + "dataset/", 'x_length_valid.npy')
    ...
    np.save(output_x_length_train, x_length_train)
    np.save(output_x_length_test, x_length_test)
    np.save(output_x_length_valid, x_length_valid)
...
def models_deal(model, X_dataset, Y_dataset, x_length_train, x_length_test, x_length_valid, y_train, y_test, y_valid):
    for index in range(len(model)):
        ...
        if model[index] == "pre-train":
            ...
            write_dataset_tsv(x_length_train, y_train, save_dir, "train")
            write_dataset_tsv(x_length_test, y_test, save_dir, "test")
            write_dataset_tsv(x_length_valid, y_valid, save_dir, "valid")
            print("finish generating pre-train's length sequence dataset.\nPlease check in %s" % save_dir)
